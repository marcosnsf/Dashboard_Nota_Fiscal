...
class DashboardController {
  ...
  async buscarIndicadoresDosCards(usuarioId) {
    const vendasFinalizadas = await Produto.countDocuments({
      situacao: "FECHADA",
      usuarioId,
    });
    const vendasAbertas = await Produto.countDocuments({
      situacao: "ABERTA",
      usuarioId,
    }); 
    const vendasAprovadas = await Produto.countDocuments({
      aprovacao: "APROVADA",
      usuarioId,
    });
    const vendasRejeitadas = await Produto.countDocuments({
      aprovacao: "REJEITADA",
      usuarioId,
    });
    return {
